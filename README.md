...
* User can start typing in the search term and get results instantly. Used debouncing function to prevent from making Api calls too many times too quickly.

* I used Giphy API to fetch 30 images in one instant. For saving the amount of data downloaded, I used downgraded still images to populate the feed. Use can then hover on the desired gif and it will render the animated gif.

* Upon reaching the end of the page, user can click on Load more button to load 30 more gifs and so on.

* If there are no results found, a warning saying "No results is found" is displayed.


## Description:

*App* : is the main component which wraps the entire application functionality.
On search, it does the api calls and passes the fetched data to GifLayout

*SearchBox* : is a functional component which is responsible for firing onChange event when user inputs text.

*GifLayout* : is responsible for displaying the list of gifs. It also has eventHandlers that execute on onMouseEnter and onMouseLeave (to detect mouse hovering over the gif)

*Item* : is a functional component which is responsible for rendering list item which shows the gif.
